...
\end{aligned}
\end{equation}
where $\mathbb{M}_e$ is the effective Mandel stress. Note that in deriving~(\ref{ineq}), we have also made us of the following identity:
\begin{equation}
...
\end{gather}
where $p$ represents the pore pressure, $\Pi_{osm}$ is the osmotic pressure of the solution. If we now substitute into Equations~(\ref{gov2}) the chemical potentials~(\ref{mu2})-(\ref{mu}), which yields to:
\begin{equation}
\begin{aligned}
...
The above equation shows that the solvent transport is driven by pressure gradient, osmotic pressure gradient (green term in Equation~(\ref{long})) and electric potential gradient. If we consider the limit $v_sC_S\rightarrow\infty$, Equations~(\ref{long}) reduces to Darcy's law for the flow in a porous media. The model for polyelectrolytes proposed by Hong \cite{Reviewpolyel} correspond instead to the limit $D^0_i\rightarrow\infty$, where mobile species can move freely in pure solution and the friction is due only to relative motion with the solid phase.
...
\end{equation}
Looking instead at the pressure gradient, we have that that the volumetric and deviatoric contribution are now decoupled:
\begin{equation}
\begin{aligned}
...
As we will discuss in the next section, when considering instead a purely volumetric, or hydrostatic, deformation, such as for free swelling, i.e. $\F=\F_{vol}=J^{1/3}\mathbb{I}$, we have that the two model are equivalent given that $G_{vol}\equiv G_A +G_B$. However, as we will be discussing in the next section, differences emerge when the deformation gradient has a non-zero deviatoric component. 
